<div ng-repeat="fund in incomeService.funds_structure">
    ...
    <div ng-if="fund.name!='Ministry Fund' && fund.name!='Special Fund'">
        <h5 class="font-bold text-muted"><% fund.name %></h5>
    </div>

    <div ng-if="fund.name=='Special Fund'">
        <a data-toggle="collapse" href="#<% fund.fund_id %>" aria-expanded="true">
            <h5 class="font-bold text-muted"><% fund.name %>
                <i class="fa fa-plus-square" ng-click="toggleSpecialFund(true)"
                   ng-show="!showSpecialFund"></i>
                <i class="fa fa-minus-square" ng-click="toggleSpecialFund(false)"
                   ng-show="showSpecialFund"></i>
            </h5>
        </a>
    </div>

    <div ng-if="fund.name=='Ministry Fund'">
        <a data-toggle="collapse" href="#<% fund.fund_id %>" aria-expanded="true">
            <h5 class="font-bold text-muted"><% fund.name %>
    ...
    <div id="<% fund.fund_id %>" ng-class="(fund.fund_id===4 || fund.fund_id===2) ? 'collapse out' : 'collapse in'">
